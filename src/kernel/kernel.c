...
char* tostr(int value) {
    static char buffer[16];  // bleibt gültig nach Funktionsende
    int i = 0;
    int neg = 0;
...
void handle_keyboard() {
    ...
    for (int i = 0; i < input_count; i++) {
        ...
        if (c == '\b') {
            if (in->len > 0) {
                in->len--;
                in->buffer[in->len+1] = 0;   // ← Letztes Zeichen löschen
            }
        } else if (c >= 32 && c <= 126) {
        ...
        vga_write_at(0, 1, tostr(in->len), 0x1F);
        vga_write_at(50, 1, in->buffer, 0x1F);
    }
}

char* getInputText(InputField *in) {
    char *intxt;
    for (int i = 0; i < in->len; i++) {
        intxt = intxt + in->buffer[i];
    }
...
void prgmSettingsBtnOkClick() {
    char *inpcol = getInputText(&inputs[0]);
    vga_write_at(50, 2, inpcol, 0x1F);

    if (str_eq(inpcol, "red")) {
        mcol = 0x14;
    } else if (str_eq(inpcol, "green")) {
        mcol = 0x12;
    } else if (str_eq(inpcol, "yellow")) {
        mcol = 0x1E;
    } else if (str_eq(inpcol, "white")) {
        mcol = 0x1F;
    } else if (str_eq(inpcol, "black")) {
        mcol = 0x10;
    } else {
        vga_write_at(0, 0, "error.", 0x14);
    }
}
...
void prgmSettings() {
    ...
    vga_clear(0x1F);
    vga_write_at(2, 2, "ChiOS System App - Einstellungen", 0x1B);

    vga_write_at(4, 6, "Mauszeigerfarbe:", 0x1F);
    vga_write_at(4, 8, "Farben: red, green, black, white, yellow", 0x1F);

    vga_write_at(4, 10, "Farbe:", 0x1F);
    add_input(12, 10, 10);
    draw_input(&inputs[0]);

    draw_button(4, 14, "Ok", 0x1E);
    add_button(4, 14, "Ok", prgmSettingsBtnOkClick);
